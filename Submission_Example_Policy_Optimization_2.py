...
# we next experiment with the following case:
# at time step t, the prediction v[t] is about the next noise w[t+1]
...
theoretical_opt_cost = np.trace(pol_opt.P - rho**2 * pol_opt.P @ H @ pol_opt.P - rho**2 * pol_opt.P @ (A - B @ pol_opt.K) @ H @ (A - B @ pol_opt.K).T @ pol_opt.P)
...
P = pol_opt.P
K = pol_opt.K
M_0_opt = - rho * np.linalg.inv(R + B.T @ P @ B) @ B.T @ P
M_1_opt = - rho * np.linalg.inv(R + B.T @ P @ B) @ B.T @ (A - B @ K).T @ P
...
for trial in range(trial_num):
    ...
    for i in trange(sys.MAX_HORIZON):
        obs = sys.observe()
        action = M_1_opt @ obs[1] - pol_opt.K @ obs[0]
        if last_obs is not None:
            action += M_0_opt @ last_obs[1]
        last_obs = obs
        sys.step(action)
